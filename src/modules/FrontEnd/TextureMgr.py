...
class TextureMgr:
    ...
    def CreateCompositeTexture(cls, name, image_paths):
        ...
        for entry in image_paths:
            # entries are either "name.png" or ("name.png", dx, dy) in scaled px
            path, dx, dy = entry if isinstance(entry, tuple) else (entry, 0, 0)
            ...
            if dx or dy:
                shifted = PILImage.new("RGBA", (w, h), (0, 0, 0, 0))
    ...
    def Initialize(cls):
        ...
        TextureMgr.CreateCompositeTexture(
            "ui_static_overlay",
            ["BG_Left_2.png", ("BG_Right_UI.png", -scale(20), 0)],
        )
        TextureMgr.CreateCompositeTexture(
